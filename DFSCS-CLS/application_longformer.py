...
import pandas as pd
import numpy as np
import csv, random, torch, models, models_longformer, transformers, time, ast
...
def evaluate_model(train_params):
    ...
    time_tag = f'DFCSC2_{model_reference}_{datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss")}'
    ...
    # loading documents
    train_file_path = '/home/hiddenleaf/AKASH_MT22007/NLP/Project/datasets/train.json'
    test_file_path = '/home/hiddenleaf/AKASH_MT22007/NLP/Project/datasets/dev.json'
    dic_docs_train = raw_to_dic_docs(train_file_path)
    dic_docs_test = raw_to_dic_docs(test_file_path)
    
    if train_params['use_malik']:
        dic_docs_train = merge_dicts(dic_docs_train, load_malik())
    ...
    if train_params.get('n_documents') is not None: # used in tests to speed up the train procedure
        ...
        temp_docs_train = {k: dic_docs_train[k] for k in list(dic_docs_train.keys())[:n_documents]}
        temp_docs_test = {k: dic_docs_test[k] for k in list(dic_docs_test.keys())[:n_documents]}
        ds_train = models.ContextualizedSC_Dataset(temp_docs_train, labels_to_idx, tokenizer, max_seq_len, min_context_len)
        ds_test = models.ContextualizedSC_Dataset(temp_docs_test, labels_to_idx, tokenizer, max_seq_len, min_context_len)
    else:
        ds_train = models.ContextualizedSC_Dataset(dic_docs_train, labels_to_idx, tokenizer, max_seq_len, min_context_len)
        ds_test = models.ContextualizedSC_Dataset(dic_docs_test, labels_to_idx, tokenizer, max_seq_len, min_context_len)
    ...
    for i, seed_val in enumerate(seeds):
        ...
        # train
        iteration_metrics, cm, train_time = models_longformer.fit(train_params, ds_train, ds_test, device)
...
def save_report(
    ...
    report = (
        ...
        f'Use Malik dataset: {train_params["use_malik"]}\n'
...
def merge_dicts(*dics):
    final_dic = {}
    for d in dics:
        for k, v, in d.items():
            if final_dic.get(k) is not None:
                raise ValueError('Duplicated key in dictionaries to be merged:', k)
            final_dic[k] = v
    return final_dic

def load_malik():
    dataset_dir = '../../Malik/RR/'
    json_files = []
    # there are two legal domains: CL and IT. The respective json files are in different diretctories
    # CL
    with open(dataset_dir + 'CL/CL_train.json') as data_file:
        json_files.append(json.load(data_file))

    with open(dataset_dir + 'CL/CL_dev.json') as data_file:
        json_files.append(json.load(data_file))

    with open(dataset_dir + 'CL/CL_test.json') as data_file:
        json_files.append(json.load(data_file))
    # IT
    with open(dataset_dir + 'IT/IT_train.json') as data_file:
        json_files.append(json.load(data_file))

    with open(dataset_dir + 'IT/IT_dev.json') as data_file:
        json_files.append(json.load(data_file))

    with open(dataset_dir + 'IT/IT_test.json') as data_file:
        json_files.append(json.load(data_file))
    
    return raw_to_dic_docs_malik(json_files)

def raw_to_dic_docs_malik(raw_data):
    map_labels = {
        'ArgumentPetitioner' : 'ARG_PETITIONER',
        'ArgumentRespondent' : 'ARG_RESPONDENT',
        'Dissent' : 'NONE',
        'Fact' : 'FAC',
        'Issue' : 'ISSUE',
        'None' : 'NONE',
        'PrecedentNotReliedUpon' : 'PRE_NOT_RELIED',
        'PrecedentOverruled' : 'PRE_NOT_RELIED',
        'PrecedentReliedUpon' : 'PRE_RELIED',
    ...
    for json_file in raw_data:
        for doc_id, values in json_file.items(): # iterates documents in current json file
            ...
            df = pd.DataFrame(list(zip(sentences, labels)), columns=['sentence', 'label'])
            if dic_.get(doc_id) is not None:
                raise ValueError('Duplicated doc ID:', doc_id)
            dic_[doc_id] = df
            # converting labels to BUILD scheme
            df['label'] = df['label'].apply(lambda l: map_labels[l])
