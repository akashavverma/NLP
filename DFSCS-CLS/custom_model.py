...
class ContextualizedSC_BERT(torch.nn.Module):
    def __init__(self, encoder_id, sep_token_id, n_classes, dropout_rate, embedding_dim, use_mlp, n_hidden_mlp=100):
        ...
        if use_mlp:
            self.dense_hidden = torch.nn.Linear(embedding_dim * 2, n_hidden_mlp)
            self.relu = torch.nn.ReLU()
            self.dense_out = torch.nn.Linear(n_hidden_mlp, n_classes)
        else:
            self.dense_out = torch.nn.Linear(embedding_dim * 2, n_classes)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # Attention pooling
        attn_weights = F.softmax(hidden_state, dim=1)
        pooled_output = torch.sum(attn_weights * hidden_state, dim=1)
        
        if self.use_mlp:
            pooled_output = self.dense_hidden(pooled_output)
            pooled_output = self.relu(pooled_output)
        
        pooled_output = self.dropout(pooled_output)
        logits = self.dense_out(pooled_output)
...
def plot_metrics(metrics):
    ...
    train_loss = [metrics[epoch][0] for epoch in epochs]
    test_loss = [metrics[epoch][1] for epoch in epochs]
    f1_micro = [metrics[epoch][4] for epoch in epochs]
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('/home/hiddenleaf/AKASH_MT22007/NLP/Project/Output/loss_plot.png')
    plt.show()

    plt.figure(figsize=(12, 6))
    plt.plot(epochs, f1_micro, label='F1 Micro Score')
    plt.title('F1 Micro Score')
    plt.xlabel('Epoch')
    plt.ylabel('F1 Micro')
    plt.legend()
    plt.grid(True)
    plt.savefig('/home/hiddenleaf/AKASH_MT22007/NLP/Project/Output/f1_score_plot.png')
    plt.show()
...
def fit(train_params, ds_train, ds_test, device):
    ...
    train_losses = []  # to store train loss at each epoch
    test_losses = []   # to store test loss at each epoch
    f1_scores = []     # to store F1 score at each epoch
    ...
    save_dir = '/home/hiddenleaf/AKASH_MT22007/NLP/Project/Output'
    save_folder = '/home/hiddenleaf/AKASH_MT22007/NLP/Project/SemEval_2023_Task_6A-main/DFCSC-CLS'
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        #storing metrics
        test_losses.append(eval_loss)   # store test loss
        f1_scores.append(f1_micro)      # store F1 score
        ...
        # print(f"Saving model at epoch {epoch} with score {f1_micro}")
        # model_file_path = os.path.join(save_folder, f"model_epoch_{epoch}.pth")
        # torch.save(train_params.state_dict(), model_file_path)
